...
There is only one script for this exercise, and the purpose of this readme according to the exercise is to explain all the scripts and copy them. Therefore, there is no much to write here as the script has comments describing every step taken. Copied below is the script. I've done the script in a way that no further changes are needed, even the file is downloaded and treated within the script so you can just run it to see how it works :) (set your working directory if you want before runing it)
...
#We declare the Url where our data is located and then download it into our working directory
...
features.labels <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/features.txt', header = FALSE, as.is = TRUE, col.names = c('id', 'name')))
activity.labels <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/activity_labels.txt', header = FALSE, as.is = c(TRUE, FALSE), col.names = c('id', 'activity.name')))
...
subjects.test.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/test/subject_test.txt', col.names = 'subject', as.is = TRUE))
y.test.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/test/y_test.txt', col.names = 'activity', as.is = TRUE))
X.test.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/test/X_test.txt', col.names = features.labels$name, as.is = TRUE))
...
subjects.train.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/train/subject_train.txt', col.names = 'subject', as.is = TRUE))
y.train.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/train/y_train.txt', col.names = 'activity', as.is = TRUE))
X.train.df <- tbl_df(read.table('./smartphoneData/UCI HAR Dataset/train/X_train.txt', col.names = features.labels$name, as.is = TRUE))
